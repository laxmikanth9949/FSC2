	...
	return Controller.extend("sap.support.fsc2.controller.BaseController", {
		...
		loadAssignedToMeSNOW: function () {
			...
			var oDataService = {
				...
				"u_escalation_type": 3,
				"sysparm_fields": "u_task_record.ref_sn_customerservice_case.account.name,active,number,correlation_display,sys_updated_by,sys_id,short_description,state,sys_updated_on,sys_class_name,approval,needs_attention"
			};
				...
				success: function (oData) {
					...
					oData.result.forEach(function (x) {
						...
						aData.push({
							...
							"account_name_F": x["u_task_record.ref_sn_customerservice_case.account.name"],
		...
		loadSnowEscalationData: function () {
			...
			var oDataService = {
				"sysparm_order": "sys_updated_on",
				"u_escalation_type": 3,
				"sysparm_fields": "u_task_record.ref_sn_customerservice_case.account.name,active,number,correlation_display,sys_updated_by,sys_id,short_description,state,sys_updated_on,sys_class_name"
			};
		...
		fillEmbeddedData: function (oData) { //Qualtrics >> activity odata
			...
			var promise = new Promise(function (resolve, reject) {
				...
				//BugFix-596
				if (that.getModel("reagionHelpModel") === undefined) {
					that.setModel(new JSONModel(jQuery.sap.getModulePath("sap.support.fsc2", "/model/ReagionHelp.json")),
						"reagionHelpModel");
				}
		...
		_processDashboardData: function () {
			var sorted = this.oAll.results;
			sorted = sorted.sort(function (a, b) {
				return b.ChangedAtNotFormatted - a.ChangedAtNotFormatted;
			});
			this.oAll = {
				"count": this.oAll.count,
				"countChanged": this.oAll.countChanged,
				"results": sorted
			};
			this.getModel("requestSet").setProperty("/results", this.oAll.results);
			this.getModel("homePageCount").setProperty("/myRequests", this.oAll.count);
